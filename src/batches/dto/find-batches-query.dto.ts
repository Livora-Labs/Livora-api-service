...
export class FindBatchesQueryDto extends PaginationQueryDto {
  @ApiPropertyOptional({ enum: BatchStatus, description: 'Estado del lote para filtrado' })
  @IsOptional()
  @IsEnum(BatchStatus, { message: 'status debe ser un valor válido del enum BatchStatus' })
  status?: BatchStatus;
  ...
  @ApiPropertyOptional({ description: 'ID del centro de acopio destino (UUID)' })
  @IsOptional()
