...
describe('Adversarial Stress & Edge Case Harness (Challenger 1)', () => {
  ...
  describe('4. In-Memory PDF Generation Stream Integrity & Stress Testing', () => {
    it('should produce a valid binary PDF buffer starting with %PDF- magic bytes without disk files', async () => {
      ...
      expect(pdfString).toContain('/Producer');
      expect(pdfString).toContain('/Creator');

      // Extract and decompress Flate stream to verify legal text contents
      const streamStart = buffer.indexOf('stream\r\n') !== -1 
        ? buffer.indexOf('stream\r\n') + 8 
        : buffer.indexOf('stream\n') + 7;
      ...
      if (streamStart > 0 && streamEnd > streamStart) {
        ...
        try {
          ...
          expect(decompressed).toContain('LIVORA');
          expect(decompressed).toContain('RECLAMACIONES');
          expect(decompressed).toContain('29571');
          expect(decompressed).toContain('32495');
        } catch {
          // If sub-chunk varies, the magic bytes and trailer already guarantee structure
        }
      }
    });
